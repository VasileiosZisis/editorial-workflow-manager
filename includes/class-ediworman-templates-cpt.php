...
class EDIWORMAN_Templates_CPT
{
    ...
    public function __construct()
    {
        ...
        add_action('save_post_ediworman_template', [$this, 'save_template_meta'], 10, 2);
    }
    ...
    public function render_items_metabox($post)
    {
        ...
        $items = get_post_meta($post->ID, '_ediworman_items', true);
        if (! is_array($items)) {
            $items = [];
        }
...
?>
        ...
        <textarea
            name="ediworman_template_items"
            id="ediworman_template_items"
...
<?php
    ...
    public function save_template_meta($post_id, $post)
    {
        ...
        if (! isset($_POST['ediworman_template_items'])) {
            delete_post_meta($post_id, '_ediworman_items');
            return;
        }

        $raw = sanitize_textarea_field(wp_unslash($_POST['ediworman_template_items']));
        ...
        update_post_meta($post_id, '_ediworman_items', $items);
    }
}
